...
// Step 1: Define the data model for an Order.
...
class Order: ObservableObject, Codable {
    ...
    var cost: Double {
        ...
        // prices
        let pricePerCupcake = 3.0
        let sprinklesCharge = 0.50
        let frostingCharge = 1.0
        
        // calculate cost
        var cost = Double(quantity) * pricePerCupcake
        if extraFrosting {
            cost += Double(quantity) * frostingCharge
        }
        if addSprinkles {
            cost += Double(quantity) * sprinklesCharge
        }
        
        // send back calculated cost
        return cost
    }
